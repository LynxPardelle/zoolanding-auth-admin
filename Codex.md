...
- 2026-06-18 01:20 CT: Voluntary TOTP enrollment is available for already signed-in BFF users through `/auth/session/mfa/enroll/start` and `/auth/session/mfa/enroll/verify`. The start step requires the active session, normal CSRF, and current password, then stores the temporary Cognito access token server-side only behind `__Host-zlp_mfa_enroll` plus `zlp_mfa_enroll_csrf`. Verification uses that temporary record, calls Cognito software-token verification and `SetUserMFAPreference`, then revokes the record and clears the enrollment cookies.
